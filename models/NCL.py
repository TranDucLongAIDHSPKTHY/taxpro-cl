...
try:
    import faiss
except ImportError:  # keep non-NCL models importable without FAISS
    faiss = None
import os
...
class NCL(nn.Module):
    ...
    def kmeans(self, embeddings):
        if faiss is None:
            raise RuntimeError(
                "NCL requires FAISS. Install faiss-gpu for CUDA or faiss-cpu "
                "for a CPU K-means fallback before running NCL."
            )
        get_num_gpus = getattr(faiss, 'get_num_gpus', lambda: 0)
        use_gpu = bool(torch.cuda.is_available() and get_num_gpus() > 0)
        kmeans = faiss.Kmeans(
            d=int(self.config['embedding_size']),
            k=self.k,
            gpu=use_gpu,
        )
